  ...
  function key(selection) {
    selection.each(function() {
      ...
      var ticks = tickValues == null ? (scale.ticks ? scale.ticks.apply(scale, tickArguments_) : scale.domain()) : tickValues,
          ...
          tick = g.selectAll('.legendcircle').data(ticks, scale1),
          tickEnter = tick.enter().insert('g', '.legend').attr('class', 'legendcircle'),
          tickExit = d3.transition(tick.exit()).remove(),
      ...
      function d3_svg_legend(selection, scale) {
        ...
        selection.select('.legendtext')
          .attr('transform', 'translate(' + (x2 + tickPadding) * sign + ', 0)');

        selection.select('.legendline')
